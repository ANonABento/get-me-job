...
export async function smartParseResume(
  ...
  if (overallConfidence < 0.5) {
    warnings.push(
      "Low confidence parse — resume may be poorly formatted or non-standard",
    );
  }
...
${sectionPrompts.join("\n\n")}`;
  ...
  try {
    ...
    return {
      ...
      llmSectionCount: 0,
      warnings: [
        `LLM enhancement failed: ${msg}. Using deterministic results only.`,
      ],
    };
  }
}
...
function mergeWithLLMResult(
  ...
  // Merge experiences
  const rawExperiences = llmResult.experience as
  ...
  // Merge education
  const rawEducation = llmResult.education as
    | Record<string, unknown>[]
    | undefined;
  ...
  // Merge projects
  const rawProjects = llmResult.projects as
    | Record<string, unknown>[]
    | undefined;
