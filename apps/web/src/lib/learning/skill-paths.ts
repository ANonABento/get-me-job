...
function analyzeSkillGaps(
  ...
  const experienceSkills = profile.experiences.flatMap((e) =>
    (e.skills || []).map(normalizeText),
  );
  ...
  return Array.from(skillGapMap.values()).sort((a, b) => {
...
function getUserSkillLevel(
  ...
  const matchingSkill = profile.skills.find(
    (s) =>
      normalizeText(s.name).includes(normalizeText(skill)) ||
...
export function generateLearningPaths(
  ...
  const paths: SkillLearningPath[] = topGaps.map((gap) => {
    ...
    const estimatedWeeks = estimateLearningWeeks(
      currentLevel || "none",
      targetLevel,
    );
    ...
    const currentLevelMapped: SkillLearningPath["currentLevel"] =
      currentLevel === "none" || currentLevel === undefined
        ? "none"
    ...
    return {
      ...
      resources: generateDefaultResources(
        gap.skill,
        targetLevel === "advanced" ? "advanced" : "intermediate",
      ),
      milestones: generateMilestones(gap.skill, targetLevel),
    };
  });

  const totalEstimatedWeeks = paths.reduce(
  ...
  if (paths.length === 0) {
    insights.push(
      "Your skills match well with available jobs! Consider deepening your expertise.",
    );
  } else {
    if (quickWins.length > 0) {
      insights.push(
        `Quick wins: Learn ${quickWins.join(", ")} first - they appear in many job listings.`,
      );
    }

    if (strategicSkills.length > 0) {
      insights.push(
        `Strategic focus: ${strategicSkills.join(", ")} are core requirements for your target roles.`,
      );
    }
    ...
    if (highPriorityCount > 0) {
      insights.push(
        `You have ${highPriorityCount} high-priority skill${highPriorityCount > 1 ? "s" : ""} to focus on.`,
      );
    }

    if (totalEstimatedWeeks > 20) {
      insights.push(
        "Consider focusing on 2-3 skills at a time rather than all at once.",
      );
    }
  }
