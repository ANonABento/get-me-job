          ...
          <div className="flex flex-col gap-3 rounded-md border border-destructive/30 bg-destructive/[0.04] p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs leading-relaxed text-ink-3">
                Clears onboarding state, builder version history, theme
                tweaks, and dashboard dismissals stored in this browser.
                Your account, opportunities, and documents on the server
                are unaffected.
              </p>
              {result.kind === "done" ? (
                <p className="mt-2 text-xs text-ink-2">
                  Cleared {pluralize(result.cleared, "local key")}. Reload
                  the page to see defaults restored.
                </p>
              ) : null}
              {result.kind === "error" ? (
