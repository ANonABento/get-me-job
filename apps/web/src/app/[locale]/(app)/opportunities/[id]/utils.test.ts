...
describe("opportunity detail utils", () => {
  ...
  it("formats list and checkbox fields for inline display", () => {
    ...
    expect(
      formatOpportunityFieldPreview(
        opportunity,
        getOpportunityFieldConfig("remote"),
      ),
    ).toBe("Yes");
  });
  ...
  it("uses option labels for select previews", () => {
    expect(
      formatOpportunityFieldPreview(
        opportunity,
        getOpportunityFieldConfig("type"),
      ),
    ).toBe("Full-time");
  });
  ...
  it("builds typed patches for editable fields", () => {
    ...
    expect(
      buildOpportunityPatch(getOpportunityFieldConfig("remote"), false),
    ).toEqual({
      remote: false,
    });
    expect(
      buildOpportunityPatch(getOpportunityFieldConfig("salary"), "  "),
    ).toEqual({
      salary: "",
    });
    expect(
